...
class TestCoreServiceRegisteredSources:
    @staticmethod
    def test_empty_registry_returns_no_sources() -> None:
        service = CoreService(registry=AdapterRegistry(), database=Database(":memory:"))

        assert not service.registered_sources()

    @staticmethod
    def test_returns_the_source_of_every_registered_adapter() -> None:
        registry = AdapterRegistry()
        registry.register(NullAdapter(Source.CLAUDE_CODE))
        registry.register(NullAdapter(Source.CURSOR))
        service = CoreService(registry=registry, database=Database(":memory:"))
